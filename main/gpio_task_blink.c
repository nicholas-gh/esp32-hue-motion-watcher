...
void init_gpio(void) {
    ...
    io_conf.pin_bit_mask = (1<<GPIO_OUTPUT_LED);
   ...
   gpio_config(&io_conf);
    ...
    gpio_set_level(GPIO_OUTPUT_LED,GPIO_SSR_OFF);

}

void gpio_task_blink(void *pvParameters) {
	EventBits_t bits;
	...
	while (1) {
		...
		for (loop=0;loop<5;loop++) {
			gpio_set_level(GPIO_OUTPUT_LED, GPIO_SSR_ON);
